import React, { useEffect, useState, useRef } from "react";
import {
  Typography,
  Avatar,
  Button,
  Grid,
  Paper,
  Card,
  CardContent,
  Fab,
} from "@mui/material";
import { useDispatch, useSelector } from "react-redux";
import { AppDispatch, RootState } from "@/store";
import UserPost from "./UserPost";
import { API } from "@/constants/api.constants";
import Image from "next/image";
import Link from "next/link";
import { fetchProfile } from "@/slice/ProfileSlice";
import { fetchtags } from "@/slice/TagsSlice";
import LocalAtmIcon from "@mui/icons-material/LocalAtm";
import { fetchData } from "@/slice/SearchSlice";
import { SealCheck } from "@phosphor-icons/react";

function getRandomColor(uniqueString: string) {
  if(!uniqueString){
    uniqueString = "A";
  }
  const hash = uniqueString.split("").reduce((acc: any, char: any) => {
    acc = char.charCodeAt(0) + ((acc << 5) - acc);
    return acc & acc;
  }, 0);

  // Calculate RGB values from the hash
  const r = (hash >> 16) & 0xff;
  const g = (hash >> 8) & 0xff;
  const b = hash & 0xff;

  // Calculate color brightness
  const brightness = (r * 299 + g * 587 + b * 114) / 1000;

  // Set text color based on brightness
  const textColor = brightness > 125 ? "#000000" : "#ffffff";

  // Return color object with background and text colors
  return {
    backgroundColor: `rgb(${r}, ${g}, ${b})`,
    color: textColor,
  };
}

const ProfilePage: React.FC = () => {
  const { profile } = useSelector((state: RootState) => state.profile);
  const [selectedProduct, setSelectedProduct] = useState<string | null>(null);
  const dispatch = useDispatch<AppDispatch>();
  const [ffcount, setFfcount] = useState<any>(0);
  const [followers, setFollowers] = useState(0);
  const { searchbase } = useSelector((state: RootState) => state.searchbase);


  useEffect(() => {
    if (profile.length === 0) {
      dispatch(fetchProfile());
      dispatch(fetchtags());
    }

    if(searchbase.length == 0){
      dispatch(fetchData());
    }

    const fetchFFC = async () => {
      try {
        const response = await fetch(
          API.BASE_URL + API.FFC.route + `/${profile.userId}`,
          {
            method: API.FFC.method,
            headers: {
              authorization: localStorage.getItem("idToken") || "",
            },
          }
        );
        const responseData = await response.json();
        setFfcount(responseData);
        setFollowers(responseData.followers);
      } catch (error) { }
    };
    fetchFFC();
  }, [dispatch, profile]);

  return (
    <div>
      <style jsx>{`
            ::-webkit-scrollbar {
              display: none; /* Hide the scrollbar */
            }
          `}</style>
      <div style={{ width: "93rem", height: "100vh", overflowY: "auto", }}>
        {profile && (
          <div style={{ display: 'flex', flexDirection: 'column', }}>
            <Card
              sx={{
                marginLeft: "2rem", marginTop: "1rem",
                height: "45vh",
                boxShadow: "rgba(0, 0, 0, 0.16) 0px 10px 36px 0px",
                width:"97%"
              }}
            >
              <div
                style={{
                  backgroundImage: `url(${API.BASE_URL}${API.GET_BG_PIC.route}/${profile.profileBackgroundPic})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                  height: "50%",
                }}
              ></div>


              <Grid item xs={3} style={{ display: "flex", marginLeft: "6rem" }}>
                {/* Profile Picture */}
                {/* <Avatar
            src={`${API.BASE_URL}${API.GET_PROFILE_PIC.route}/${profile.profilePic}`}
            alt={profile.name}
            style={{ width: "180px", height: "180px", marginTop:"-6rem"}}
          /> */}
                <Avatar
                  src={`${API.BASE_URL}${API.GET_PROFILE_PIC.route}/${profile.profilePic}`}
                  alt={profile.name}
                  style={{
                    width: "180px",
                    height: "180px",
                    marginTop: "-6rem",
                    marginLeft: "-2.5rem",
                    ...getRandomColor(profile.userId),
                    fontSize: "4rem",
                    fontWeight: "bold",
                    textTransform: "uppercase",
                  }}
                >
                  {profile.name ? profile.name.charAt(0) : ""}
                </Avatar>
              </Grid>
              <Grid
                container
                rowSpacing={1}
                columnSpacing={{ xs: 1, sm: 2, md: 3 }}
              >
                <Grid item xs={2}>
                  <div style={{ whiteSpace: "nowrap", marginTop: "2rem", paddingLeft: "4rem" }}>
                    <Typography
                      variant="h3"
                      style={{ fontSize: "var(--fs-heading)" }}
                    >
                      <span className="gradient-text">{profile.name}</span>
                      {profile.role === "titan" && (
                        <SealCheck size={32} color="#00a3f5" weight="fill" />
                      )}
                    </Typography>
                    <Typography
                      variant="subtitle1"
                      style={{ fontSize: "var(--fs-normal)" }}
                    >
                      <span className="gradient-text">{profile.email}</span>
                    </Typography>
                  </div>
                </Grid>
                <Grid
                  item
                  xs={3}
                  style={{
                    padding: "0rem",
                    marginLeft: "22rem",
                    marginTop: "-2rem",
                  }}
                >
                  {/* About Me */}
                  <Typography
                    variant="h6"
                    className="gradient-text"
                    style={{ fontSize: "var(--fs-normal)" }}
                  >
                    About Me
                  </Typography>
                  <div style={{ overflowY: 'auto', maxHeight: '6rem', }}>
                  <Typography style={{
                    fontSize: "var(--fs-normal)", wordWrap: "break-word",
                    overflowWrap: "break-word",
                    hyphens: "auto", overflow: "hidden",
                    textOverflow: "ellipsis",
                  }}>
                    {profile.aboutMe}
                  </Typography>
                  </div>
                </Grid>
                <Grid
                  container
                  spacing={1.5}
                  style={{
                    width: "30%",
                    marginTop: "-2.5rem",
                    marginLeft: "2rem",
                  }}
                >
                  <Grid item xs={6}>
                    <Typography
                      variant="h5"
                      className="gradient-text"
                      style={{ fontSize: "var(--fs-sub-heading)", textAlign: "center" }}
                    >
                      {followers}
                    </Typography>
                    <Typography style={{ fontSize: "var(--fs-normal)", textAlign: "center" }}>
                      Followers
                    </Typography>
                  </Grid>

                  <Grid item xs={6}>
                    <Typography
                      variant="h5"
                      className="gradient-text"
                      style={{ fontSize: "var(--fs-sub-heading)", textAlign: "center" }}
                    >
                      {ffcount?.following}
                    </Typography>
                    <Typography style={{ fontSize: "var(--fs-normal)", textAlign: "center" }}>
                      Following
                    </Typography>
                  </Grid>
                </Grid>
              </Grid>

            </Card>
            <Card
              sx={{
                padding: "2rem",
                paddingTop: "0.5rem",
                marginTop: "2rem",
                marginLeft: "2rem",
                height: "50rem",
                overflowY: "auto",
                width:"93%",
                boxShadow: "rgba(0, 0, 0, 0.25) 0px 25px 50px -12px",
                scrollbarWidth: "thin",
                scrollbarColor: "transparent transparent",
                "&::-webkit-scrollbar": {
                  width: "6px",
                },
                "&::-webkit-scrollbar-thumb": {
                  background: "transparent",
                },
              }}
            >
              <UserPost
                profile={profile}
                prodName={"all"}
                privateProfile={true}
              />

            </Card>

          </div>
        )}
      </div>
    </div>
  );
};

export default ProfilePage;
